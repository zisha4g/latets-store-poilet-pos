...
export function useDataManagement(user, isDemo = false) {
    ...
    try {
      const fetchPromises = tables.map(table => {
        if (table === 'saved_carts') {
          return supabase.from(table).select('*, customers(*)').eq('user_id', user.id);
        }
      ...
      setData({
        ...
        appointments: fetchedData.appointments || [],
  ...
  const handlers = useMemo(() => {
    ...
    return {
      ...
      deliveries: { ...createHandlers('deliveries') },
      appointments: { ...createHandlers('appointments') },
      tasks: { ...createHandlers('tasks') },
      taxes: { 
