...
const PosView = ({ products, handlers, settings, savedCarts = [] }) => {
  ...
  const { subtotal, discountAmount, totalAfterDiscount, appliedTaxes, appliedServiceCharges, total } = useMemo(() => {
    ...
    // Debug: Check if handlers.taxes exists and what it returns
    console.log('🔍 Tax Debug - handlers.taxes exists:', !!handlers?.taxes);
    console.log('🔍 Tax Debug - getApplicable exists:', !!handlers?.taxes?.getApplicable);
    
    const taxes = (handlers?.taxes && handlers.taxes.getApplicable) ? handlers.taxes.getApplicable(totalAfterDisc, saleState.cart) : [];
    
    console.log('🔍 Tax Debug - Applied taxes:', taxes);
    console.log('🔍 Tax Debug - Cart items:', saleState.cart);
    console.log('🔍 Tax Debug - Total after discount:', totalAfterDisc);
